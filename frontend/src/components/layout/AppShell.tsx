      ...
      <div className="mx-auto flex min-h-screen max-w-[1600px] gap-6 px-4 py-6 lg:px-10">
        <aside className="hidden w-80 shrink-0 rounded-[2rem] border border-white/10 bg-slate-900/70 p-6 shadow-glow backdrop-blur-xl lg:block">
              ...
              <NavLink
                ...
                className={({ isActive }) =>
                  `block rounded-3xl px-4 py-3 text-sm font-medium transition ${
                    isActive ? 'bg-slate-800 text-white shadow-glow' : 'text-slate-300 hover:bg-white/5'
                  }`
                }
              >
                {item.name}
          ...
          <button
            ...
            className="mt-8 w-full rounded-3xl border border-slate-700 bg-slate-950/80 px-4 py-3 text-sm text-slate-200 transition hover:bg-slate-900"
          >
            Logout
          </button>
        </aside>
