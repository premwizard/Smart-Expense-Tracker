    ...
    return (<div className="space-y-8">
      <div className="rounded-[2rem] border border-white/10 bg-slate-950/80 p-6 shadow-glow">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <input type="search" placeholder="Search transactions" value={search} onChange={(e) => setSearch(e.target.value)} className="rounded-3xl border border-white/10 bg-slate-950/80 px-4 py-3 text-slate-100 outline-none transition focus:border-sky-400"/>
            <select value={category} onChange={(e) => setCategory(e.target.value)} className="rounded-3xl border border-white/10 bg-slate-950/80 px-4 py-3 text-slate-100 outline-none transition focus:border-sky-400">
              {categories.map((item) => (<option key={item} value={item}>{item}</option>))}
            </select>
            <select value={sortKey} onChange={(e) => setSortKey(e.target.value)} className="rounded-3xl border border-white/10 bg-slate-950/80 px-4 py-3 text-slate-100 outline-none transition focus:border-sky-400">
      ...
      <div className="overflow-hidden rounded-[2rem] border border-white/10 bg-slate-950/80 shadow-glow">
        <table className="w-full border-collapse text-left text-sm text-slate-300">
          ...
          <tbody>
            {filtered.slice(0, 12).map((item) => (<tr key={item.id} className="border-t border-white/5 hover:bg-slate-900/80">
        ...
        {!filtered.length && (<div className="p-8 text-center text-slate-400">No transactions match your criteria. Try another filter.</div>)}
      </div>
    </div>);
};
