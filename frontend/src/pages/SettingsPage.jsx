...
const SettingsPage = () => {
    ...
    useEffect(() => {
        const stored = localStorage.getItem('aet_theme');
        if (stored) {
            setTheme(stored);
            ...
            <div className="space-y-3">
              <div>
                ...
                <div className="mt-4 flex items-center gap-4">
                  <button onClick={() => handleThemeChange('dark')} className={`rounded-3xl px-4 py-3 text-sm ${theme === 'dark' ? 'bg-sky-400 text-slate-950' : 'bg-slate-950/80 text-slate-300'}`}>
                    Dark
                  </button>
                  <button onClick={() => handleThemeChange('light')} className={`rounded-3xl px-4 py-3 text-sm ${theme === 'light' ? 'bg-sky-400 text-slate-950' : 'bg-slate-950/80 text-slate-300'}`}>
                    Light
                  </button>
                </div>
              </div>
