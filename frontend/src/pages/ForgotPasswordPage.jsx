...
const ForgotPasswordPage = () => {
    ...
    return (<AuthLayout title="Reset your password" subtitle="Enter your email and we'll send you secure reset instructions.">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <label className="block text-sm text-slate-300">
          Email address
          <input type="email" {...register('email', { required: true })} className="mt-2 w-full rounded-3xl border border-white/10 bg-slate-950/80 px-4 py-4 text-slate-100 outline-none transition focus:border-sky-400"/>
        </label>
