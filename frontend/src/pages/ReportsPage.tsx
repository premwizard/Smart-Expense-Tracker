    ...
    <div className="space-y-8">
      ...
      <div className="rounded-[2rem] border border-white/10 bg-slate-950/80 p-6 shadow-glow">
        ...
        <div className="mt-6 space-y-4">
          {(reports.length ? reports : [{ report_type: 'Monthly', period: 'Jun 2026', generated_at: '2026-06-05' }]).map((report, index) => (
            <div key={index} className="rounded-3xl border border-white/10 bg-slate-900/80 p-5">
              ...
              {report.download_url && (
                <a href={report.download_url} className="mt-4 inline-flex rounded-full bg-sky-400 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-sky-300">
                  Download
                </a>
              )}
            </div>
