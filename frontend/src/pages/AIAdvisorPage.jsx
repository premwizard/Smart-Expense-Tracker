...
const AIAdvisorPage = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setTimeout(() => {
            setReply('Your highest spending is concentrated in Rent and Shopping. Stop-and-think shopping notes could reduce non-essential expenses by 12%, and moving select subscriptions to quarterly billing would increase your monthly savings.');
            setLoading(false);
        }, 900);
    };
    return (<div className="space-y-8">
      ...
      <div className="grid gap-6 xl:grid-cols-[0.9fr_0.55fr]">
        <div className="rounded-[2rem] border border-white/10 bg-slate-950/80 p-6 shadow-glow">
          <form onSubmit={handleSubmit} className="space-y-6">
            <label className="block text-sm text-slate-300">
              Ask a question
              <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={6} className="mt-2 w-full rounded-3xl border border-white/10 bg-slate-950/80 px-4 py-4 text-slate-100 outline-none transition focus:border-sky-400" placeholder="Where am I overspending this month?"/>
            </label>
        ...
        <div className="space-y-4">
          {questions.map((item) => (<button key={item} onClick={() => setPrompt(item)} className="w-full rounded-3xl border border-white/10 bg-slate-950/80 px-6 py-5 text-left text-sm text-slate-100 transition hover:border-sky-400">
              {item}
            </button>))}
          <div className="rounded-[2rem] border border-white/10 bg-slate-950/80 p-6 shadow-glow">
