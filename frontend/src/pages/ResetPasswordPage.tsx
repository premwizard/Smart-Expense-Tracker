...
const ResetPasswordPage = () => {
  ...
  const { register, handleSubmit } = useForm<ResetPayload>();
  const [message, setMessage] = useState('');
  const [loading, setLoading] = useState(false);
  const accessToken = searchParams.get('access_token');

  const onSubmit = async ({ password }: ResetPayload) => {
    setLoading(true);
    setMessage('');
  ...
  return (
    <AuthLayout title="Enter a new password" subtitle="Secure your account with a strong password and continue your journey." >
