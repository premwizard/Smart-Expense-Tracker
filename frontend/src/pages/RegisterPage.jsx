...
const RegisterPage = () => {
    ...
    return (<AuthLayout title="Create your account" subtitle="Register with your email and secure your finance workspace with AI analytics.">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        <label className="block text-sm text-slate-300">
          Full name
          <input type="text" {...register('name', { required: 'Name is required' })} className="mt-2 w-full rounded-3xl border border-white/10 bg-slate-950/80 px-4 py-4 text-slate-100 outline-none transition focus:border-sky-400"/>
        </label>
        <label className="block text-sm text-slate-300">
          Email
